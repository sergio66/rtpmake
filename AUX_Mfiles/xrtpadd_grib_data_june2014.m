...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  if exist([sourcename '.inv']) & exist([sourcename])
    ...
    if ~( max(gribdate)+0.5/rec_per_day > min(rtime) & min(gribdate)-0.5/rec_per_day < max(rtime) )
      %[min(rtime) max(gribdate)+0.5/rec_per_day (max(gribdate)+0.5/rec_per_day)-min(rtime) 0]
      %[max(rtime) min(gribdate)-0.5/rec_per_day max(rtime)-(min(gribdate)-0.5/rec_per_day) 0]
