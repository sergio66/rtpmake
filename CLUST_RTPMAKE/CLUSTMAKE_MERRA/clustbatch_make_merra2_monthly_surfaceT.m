...
fname = ['/asl/models/merra2_monthly/' num2str(yy) '/merra2_' num2str(yy) num2str(mm,'%02d') '_sfc.nc'];
...
for jj = 1 : 64
  ...
  for ii = 1 : 72
    ...
    wah = a.skt(booX,booY);
    wah = wah(:);
    mean_stempIJ(ii,jj) = nanmean(wah);
...
figure(1); scatter_coast(rlonIJ,rlatIJ,50,mean_stempIJ); colormap jet
figure(2); scatter_coast(rlonIJ,rlatIJ,50,cntr_stempIJ); colormap jet
figure(3); scatter_coast(rlonIJ,rlatIJ,50,mean_stempIJ - cntr_stempIJ); colormap(usa2); caxis([-1 +1])
