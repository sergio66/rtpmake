...
% clustcmd -L clust_make_ecmcloudrtp_sergio_pcrtm_YYMMDD_loop16days_10deg.m 001:240
%
% otherwise when happy
% clustcmd -q medium -n 64 -p 4 clust_make_ecmcloudrtp_sergio_pcrtm_YYMMDD_loop16days_10deg.m 001:240
%
% or
% clustcmd -q long_contrib -n 64 -p 4 clust_make_ecmcloudrtp_sergio_pcrtm_YYMMDD_loop16days_10deg.m 001:240
...
icestr = ['cloudy_airs_l1b_ecm_centerfovs' icestr '.'];
...
while yy0 == 2007
  ...
  if eeP == 0
    ...
    woo = find(abs(p.scanang) <= 10);
    fprintf(1,'orig numfovs = %5i \n',length(p.scanang));
    [h,p] = subset_rtp_clouds(h,p,[],[],woo);
    fprintf(1,'final numfovs = %5i \n',length(p.scanang));
    ...
    p2x.rcalc = p2x.rad_allsky;
    p2x = rmfield(p2x,'rad_allsky');
