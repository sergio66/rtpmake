...
run_sarta.sartacloud_code = sartasct_bin.shis;
run_sarta.sartaclear_code = sartasct_bin.shis;
run_sarta.klayers_code = klayers_shis;

% this is set in calling routine cloud_set_defaults_run_makerBLAH.m

%% icestr = ['NEWLANDFRAC/cloudy_shis_l1b_ecm' icestr '.'];
if iv5or6 == 5
  icestr = ['cloudy_shis_ecm' icestr '.'];
elseif iv5or6 == 6
  icestr = ['cloudy_shis_ecm' icestr '.'];
end
...
for ixx = 1 : length(iaGlist)
  ...
  if eeP == 0
    ...
    path2SHIS = '/home/sergio/nogit/WHYMSIE/';
    filename = [path2SHIS 'whymsie_shis_rad_ER2_' num2str(year,'%04d') num2str(month,'%02d') num2str(day,'%02d') '_R0.nc'];
    a = read_netcdf_lls(filename);

    p.rtime   = utc2taiSergio(year*ones(1,length(a.Altitude)),month*ones(1,length(a.Altitude)),day*ones(1,length(a.Altitude)),double(a.timeUTC'));
    p.rlat    = a.Latitude';
    p.rlon    = a.Longitude';    
    p.zobs    = a.Altitude';
    p.zobsBar = a.barometricAltitude';
    p.scanang = a.FOVangle';
    p.nesr    = a.HBB_NESR * ones(1,length(p.scanang));
    p.pobs    = a.aircraftExtPressure';
    p.robs1   = a.radiance;
    p.solzen = 150 * ones(size(p.rlat));
    
    p.upwell = ones(size(p.solzen));
    %p.irinst = AIRSinst*ones(1,nobs);
    %p.findex = grannum*ones(1,nobs);

    scatter_coast(p.rlon,p.rlat,2,p.zobs); title('Colorbar = zobs'); colormap jet

    bad = find(isnan(p.rtime) | isnan(p.rlon) | isnan(p.rlat));
    if length(bad) > 0    
      good = setdiff(1:length(p.rtime),bad);
      p.rtime = p.rtime(good);
      p.rlat = p.rlat(good);
      p.rlon = p.rlon(good);
      p.zobs = p.zobs(good);
      p.zobsBar = p.zobsBar(good);      
      p.scanang = p.scanang(good);
      p.pobs = p.pobs(good);
      p.solzen = p.solzen(good);
      p.upwell = p.upwell(good);
      p.nesr   = p.nesr(:,good);
      p.robs1  = p.robs1(:,good);      
    end
    ...
    if iv5or6 == 5
      ...
      h.vchan = a.wavenumber;
      h.ichan = (1 : length(h.vchan))';
      h.nchan = length(h.vchan);
    end

    % see set_landfrac_using_L1B_L1C_or_usgs
...
%p00 = p
   ...
   %% we have ichan wrong!
    ...
    if run_sarta.cloud < 0
      ...
      scatter_coast(p2x.rlon,p2x.rlat,10,p.stemp-tobs1231)
    else
      i1231 = find(h.vchan >= 1231,1);
      tobs1231 = real(rad2bt(h.vchan(i1231),p2x.robs1(i1231,:)));
...
if iERA5 > 0
   p5 = p; p5 = rmfield(p5,'ptemp'); p5 = rmfield(p5,'gas_1'); p5 = rmfield(p5,'gas_3'); p5 = rmfield(p5,'stemp');
   p5 = rmfield(p5,'cc'); p5 = rmfield(p5,'clwc'); p5 = rmfield(p5,'ciwc'); p5 = rmfield(p5,'nlevs'); p5 = rmfield(p5,'plevs');
   [p5,h5] = fill_era5_daily(p5,h);

   [xyy,xmm,xdd,xhh] = tai2utcSergio(p5.rtime);        %%% <<<<<<<<<<<<<<<<<<<<<<<<<<<<< for SdSM old time
   time_so_far = (xyy-2000) + ((xmm-1)+1)/12;
   co2ppm = 368 + 2.077*time_so_far;  %% 395.6933
   p5.co2ppm = co2ppm;
   [p25] = driver_sarta_cloud_rtp(h,ha,p5,pa,run_sarta);

   [h,ha,p2x5,pa] = rtptrim_sartacloud(h,ha,p25,pa);

   bah = strfind(fnamex,'shis_ecm');
   fnamex5 = [fnamex(1:bah-1) 'shis_era5' fnamex(bah+length('shis_ecm'):end)];
   
   rtpwrite(fnamex5,h,ha,p2x5,pa)
   fprintf(1,'saved %s \n',fnamex5)
