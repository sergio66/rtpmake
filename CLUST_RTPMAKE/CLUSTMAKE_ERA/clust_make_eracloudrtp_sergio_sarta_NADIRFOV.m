...
% clustcmd -L clust_make_eracloudrtp_sergio_sarta_NADIRFOV.m yy0mm0dd0:yyEmmEddE
%
% otherwise when happy
% clustcmd -q medium -n 64 -p 4 clust_make_eracloudrtp_sergio_sarta_NADIRFOV.m yy0mm0dd0:yyEmmEddE
%
% or
% clustcmd -q long_contrib -n 64 -p 4 clust_make_eracloudrtp_sergio_sarta_NADIRFOV.m yy0mm0dd0:yyEmmEddE
...
icestr = ['cloudy_airs_l1b_era_nadirfov' icestr '.'];
...
for ixx = 1 : length(iaGlist)
  ...
  if eeP == 0
    ...
    [h,ha,p,pa] = rtpadd_era_data(h,ha,p,pa,cldfields); %%% add on ecm
    p0 = p;
    ...
    pALL = p;
    nadir = find(p.xtrack == 45 | p.xtrack == 46);
    [h,p] = subset_rtp_clouds(h,p,[],[],nadir);
