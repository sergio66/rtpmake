...
%% copied/modified from clust_ecmcloudrtp_sergio_sarta_YYMMDD_loopGG_loop16days.m

% local running to test
% clustcmd -L clust_ecmcloudrtp_sergio_sarta_YYMMDD_loopGG_loop16daysPERT.m 001:240
%
% otherwise when happy
% clustcmd -q medium -n 128 clust_ecmcloudrtp_sergio_sarta_YYMMDD_loopGG_loop16daysPERT.m 001:240
%
% or
% clustcmd -q long_contrib -n 128 clust_ecmcloudrtp_sergio_sarta_YYMMDD_loopGG_loop16daysPERT.m 001:240
...
%% pert = PTFAS;    % P = phase (1=water,2=ice) , T=cprtop,  F=fraction,  A=amount, S=size

pertSTR = '12000';   %% water cloud, perturb cprtop by 10%
pertSTR = '11000';   %% water cloud, perturb cprtop by 10%
pertSTR = '10100';   %% water cloud, perturb fraction
pertSTR = '10010';   %% water cloud, perturb amount
pertSTR = '10001';   %% water cloud, perturb size
 
pertSTR = '21000';   %% ice cloud, perturb cprtop by 10%
pertSTR = '20100';   %% ice cloud, perturb fraction
pertSTR = '20010';   %% ice cloud, perturb amount
pertSTR = '20001';   %% ice cloud, perturb size
...
icestr = ['cloudy_airs_l1b_ecm' icestr '_pert_' num2str(pert) '.'];
...
while yy0 == 2007
  ...
  if eeP == 0 & eeP0 > 0
    ...
    %% no need to do any of this, as we have already matched ECM to AIRS
    %{   
...
% >>>>>>>>>>>>>>>>>>>>>>>>>
    ...
    [h,ha,p,pa] = rtpread(fnameOUT0);
    [h,ha,p,pa] = rtpadd_emis_wis(h,ha,p,pa);

    pnew = perturb_slabcloud(h,ha,p,pa,pertSTR,klayers,run_sarta.sartacloud_code);
    rtpwrite(fnameOUT,h,ha,pnew,pa)
