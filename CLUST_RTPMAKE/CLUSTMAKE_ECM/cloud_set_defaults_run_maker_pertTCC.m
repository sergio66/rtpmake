...
if iv5or6 == 5
  icestr = ['pert_tcc_cloudy_airs_l1b_ecm' icestr '.'];
elseif iv5or6 == 6
  icestr = ['pert_tcc_cloudy_airs_l1c_ecm' icestr '.'];
end
...
for ixx = 1 : length(iaGlist)
  ...
  pstr = ['_pert_tcc_model_' num2str(iPertTCC)];
  ...
  if iSlabCld_CumSumStrowORGeorge == 1
    fnameOUT= [fdirOUT icestr ystr '.' mstr '.' dstr '.' gstr pstr '.rtp'];
  else
    fnameOUT= [fdirOUT icestr ystr '.' mstr '.' dstr '.' gstr pstr '_cumsum_-1.rtp'];
  end
  ...
  if eeP == 0
    ...
    if iPertTCC == 1
      ...
      p.tcc = tcc1;
    elseif iPertTCC == 2    
      disp('replacing NWP tcc with method2 tcc')
      tcc2 = tcc_method2(p);
      p.tcc0 = p.tcc;
      p.tcc = tcc2;
    elseif iPertTCC == 3
      disp('replacing NWP tcc with method3A tcc')
      [tcc3A,tcc3B] = tcc_method3(p);
      p.tcc0 = p.tcc;
      p.tcc = tcc3A;
    elseif iPertTCC == 4
      disp('replacing NWP tcc with method3B tcc')
      [tcc3A,tcc3B] = tcc_method3(p);
      p.tcc0 = p.tcc;
      p.tcc = tcc3B;
    end
