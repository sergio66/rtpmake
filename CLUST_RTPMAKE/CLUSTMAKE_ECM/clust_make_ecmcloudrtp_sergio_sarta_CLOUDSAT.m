...
% clustcmd -L clust_make_ecmcloudrtp_sergio_sarta_CLOUDSAT.m yy0mm0dd0:yyEmmEddE
%
% otherwise when happy
% clustcmd -q medium -n 64 -p 4 clust_make_ecmcloudrtp_sergio_sarta_CLOUDSAT.m yy0mm0dd0:yyEmmEddE
%
% or
% clustcmd -q long_contrib -n 64 -p 4 clust_make_ecmcloudrtp_sergio_sarta_CLOUDSAT.m yy0mm0dd0:yyEmmEddE
...
yymmdd0 = [str2num(strx(1:4)) str2num(strx(5:6)) str2num(strx(7:8))];
...
icestr = ['cloudy_airs_l1b_ecm_cloudsat' icestr '.'];
...
for ixx = 1 : length(iaGlist)
  ...
  if eeP == 0
    ...
    %% now SUBSET ixtrac=45:46
    pALL = p;
    center = find(p.xtrack == 45 | p.xtrack == 46);
    center = find(p.xtrack == 45+7 | p.xtrack == 46+7);  %% daytime
    center = find(p.xtrack == 45-14 | p.xtrack == 46-14);  %% nighttime
    [h,p] = subset_rtp_clouds(h,p,[],[],center);
