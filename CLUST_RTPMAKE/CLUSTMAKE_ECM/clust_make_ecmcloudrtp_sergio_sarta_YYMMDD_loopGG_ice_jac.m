...
% clustcmd -L clust_make_ecmcloudrtp_sergio_sarta_YYMMDD_loopGG_ice_jac.m 001:240
%
% otherwise when happy
% clustcmd -q medium -n 64 -p 4 clust_make_ecmcloudrtp_sergio_sarta_YYMMDD_loopGG_ice_jac.m 001:240
%
% or
% clustcmd -q long_contrib -n 64 -p 4 clust_make_ecmcloudrtp_sergio_sarta_YYMMDD_loopGG_ice_jac.m 001:240
...
icestr  = ['cloudy_airs_l1b_ecm_ice_jac'  icestr '.'];
...
for ixx = 1 : length(iaGlist)
  ...
  if eeP > 0 & ee0 > 0 & ee == 0
    ...
    %% doing ICE jacobians
    iw = find(p.ctype  == 201); p.cngwat(iw)  = p0.cngwat(iw) * 1.1;
    iw = find(p.ctype2 == 201); p.cngwat2(iw) = p0.cngwat2(iw) * 1.1;
