...
rrlat = -89 : 1 : +89;
rrlon = -179 : 1 : +179;
[xlat,xlon] = meshgrid(rrlat,rrlon);
xlat = xlat(:);
xlon = xlon(:);
plot(xlon,xlat,'.'); whos xlat xlon
pp.rlat = xlat';
pp.rlon = xlon';
pp.rtime = p.rtime(1) * ones(size(pp.rlat));

    [pp,h] = fill_ecmwf(pp,h);

fnameOUT = 'test_for_paper1.ip.rtp';
rtpwrite(fnameOUT,h,[],pp,[]);
klayerser = ['!' klayers ' fin=' fnameOUT ' fout=test_for_paper1.op.rtp'];
eval(klayerser)

[haha,~,papa,~] = rtpread('test_for_paper1.op.rtp');
usstd = load('/home/sergio/SPECTRA/IPFILES/std_water');
usstd = flipud(usstd);

figure(1);
semilogy(papa.ptemp,papa.plevs,'b',usstd(:,4),1013.25*usstd(:,2),'r.-',...
         usstd(:,4)-50,1013.25*usstd(:,2),'r',usstd(:,4)+50,1013.25*usstd(:,2),'r')
set(gca,'ydir','reverse')
axis([150 350  0.005 1020]); grid on

do_the_usstd_stats

error('lkjaflkjafxxxx')
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
