function [hd0, hattr, pd0, pattr, tstr] = cris_l1c_to_rtp_sergio_ecm_or_era5(yy,mm,dd,rgrans,NWPmodel,iSNPPorJ1orJ2,iInterp,iUVW)
...
for fn = iign
 ...
 %% plot(h.vchan,noise)
  ...
  switch cfg.model
   ...
   case 'era5'
     if iInterp <= 0
       [p,h,pattr]  = fill_era5_daily(p,h,pattr);
       %[p,h,pattr]  = fill_era5_interp(p,h,pattr);
     else
       [p,h,pattr]  = fill_era5_daily(p,h,pattr);       
       %new_4_ERA5files_interp_analysis
     end
  ...
  %{
  figure(1); scatter_coast(p.rlon,p.rlat,20,p.stemp);
  figure(2); scatter_coast(p.rlon,p.rlat,20,p.pblh);
  %}
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
