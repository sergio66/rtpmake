function F = grib_interpolate_era5x(fn_s,fn_h,hindex);
% 
...
%         hindex is the hour index (8 total) in the era5 files
...
u = permute(single(ncread(fn_h,'u',[1 1 1 hindex],[Inf Inf Inf 1])),[2,1,3]);
for i=1:length(F.levid)
   F.u(i).ig = griddedInterpolant(iX,iY,flipud(squeeze(u(:,:,i))),'linear');
end
clear u

v = permute(single(ncread(fn_h,'v',[1 1 1 hindex],[Inf Inf Inf 1])),[2,1,3]);
for i=1:length(F.levid)
   F.v(i).ig = griddedInterpolant(iX,iY,flipud(squeeze(v(:,:,i))),'linear');
end
clear v

w = permute(single(ncread(fn_h,'w',[1 1 1 hindex],[Inf Inf Inf 1])),[2,1,3]);
for i=1:length(F.levid)
