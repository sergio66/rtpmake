...
figure(10); dst = (-1:0.05:+1)*0.025; plot(dst,histc(skt_trend_mean-skt_trend_Q90,dst),'b',dst,histc(skt_trend_mean-skt_trend_cntr,dst),'r'); plotaxis2;
...
aslmap(2,rlat65,rlon73,smoothn((reshape(skt_trend_Q90',72,64)')  ,1), [-90 +90],[-180 +180]); caxis([-1 +1]*0.15); colormap(llsmap5); title('ERA5 SKT trend Q90')
aslmap(3,rlat65,rlon73,smoothn((reshape(skt_trend_cntr',72,64)') ,1), [-90 +90],[-180 +180]); caxis([-1 +1]*0.15); colormap(llsmap5); title('ERA5 SKT trend cntr')
aslmap(4,rlat65,rlon73,(reshape(skt_trend_mean',72,64)'), [-90 +90],[-180 +180]); caxis([-1 +1]*0.15); colormap(llsmap5); title('ERA5 SKT trend mean NO SMOOTH')
aslmap(5,rlat65,rlon73,(reshape(skt_trend_Q90',72,64)'), [-90 +90],[-180 +180]); caxis([-1 +1]*0.15); colormap(llsmap5); title('ERA5 SKT trend Q90 NO SMOOTH')
aslmap(6,rlat65,rlon73,(reshape(skt_trend_cntr',72,64)'), [-90 +90],[-180 +180]); caxis([-1 +1]*0.15); colormap(llsmap5); title('ERA5 SKT trend cntr NO SMOOTH')
