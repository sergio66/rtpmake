...
%% same as clust_make_ecmcloudrtp_sergio_sarta_YYMMDD_loopGG.m except we SEPARET ice N water clouds
%% using run_sarta.ice_water_separator = 440; 

% local running to test
% clustcmd -L clust_make_AIRSL2_90x135_cloudrtp_YYMMDD_loopGG.m 001:240
%
% otherwise when happy
% clustcmd -q medium -n 64 clust_make_AIRSL2_90x135_cloudrtp_YYMMDD_loopGG.m 001:240
%
% or
% clustcmd -q long_contrib -n 64 clust_make_AIRSL2_90x135_cloudrtp_YYMMDD_loopGG.m 001:240
...
for ixx = 1 : length(iaGlist)
  ...
  if iL2CldType == -1
    fnameOUT= [dirOUT 'allfov_airs_cloudNprofL2data.' ystr '.' mstr '.' dstr '.' gstr '_v6.rtp'];
  elseif iL2CldType == +1
    fnameOUT= [dirOUT 'allfov_airs_cloudNprofL2data_Joel.' ystr '.' mstr '.' dstr '.' gstr '_v6.rtp'];
  end
  ...
  if eeP == 0
    ...
    %eval(toucher)
    if iL2CldType == -1
      combine_cc_l2ret_v6_cloudsKahn_sergio_90x135(yy,mm,dd,gg,dirOUT);
    elseif iL2CldType == +1
      combine_cc_l2ret_v6_cloudsJoel_sergio_90x135(yy,mm,dd,gg,dirOUT);
    end
  else
